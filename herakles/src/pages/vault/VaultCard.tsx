...
export const VaultCard = ({ card, img_locker, img_edit, handleOpen }) => {
  return (
    <Card isPressable className="w-[400px] space-y-5 m-2 vault-container" radius="lg" key={card.uid}>
      <div className='w-full flex flex-row items-center p-4 vault-container-top'>
        <img width={60} alt={card.name} src={img_locker} />
        ...
        <div className="h-full" style={{ height: 64 }}>
          <img width={50} alt={card.name} src={img_edit} className='vault-container-editor-icon' onClick={() => handleOpen(card.uid)} />
        </div>
      </div>
      <div className='w-full flex px-2'>
        <SimpleBar data-simplebar-auto-hide="false" className='w-full flex flex-col items-start px-2' style={{ height: 70 }}>
          ...
          { card.hasOwnProperty('description') && card.description !== ""  ? (
